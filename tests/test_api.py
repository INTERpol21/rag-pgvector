...
async def test_api_is_served_under_v1_only(client):
    """The API lives under /v1; the old unprefixed paths are gone (404).

    Guards the /v1 unification: /v1/ingest is the only ingest path, /ingest no
    longer resolves, and the OpenAPI schema advertises only /v1 routes.
    """
    doc = {"documents": [{"title": "t", "text": "vector search with pgvector"}]}
    assert (await client.post("/v1/ingest", json=doc)).status_code == 200
    assert (await client.post("/ingest", json=doc)).status_code == 404

    paths = (await client.get("/openapi.json")).json()["paths"]
    assert {"/v1/ingest", "/v1/query", "/v1/stats"} <= set(paths)
    assert not any(p in paths for p in ("/ingest", "/query", "/stats"))


async def test_ingest_validation_422(client):
    # empty document list
    resp = await client.post("/v1/ingest", json={"documents": []})
    assert resp.status_code == 422
    # missing required fields
    resp = await client.post("/v1/ingest", json={"documents": [{"title": "no text"}]})
    assert resp.status_code == 422
    # empty text
    resp = await client.post(
        "/v1/ingest", json={"documents": [{"title": "t", "text": ""}]}
    )
    assert resp.status_code == 422
...
async def test_query_validation_422(client):
    ...
    resp = await client.post("/v1/query", json={"question": "ok", "top_k": 0})
    assert resp.status_code == 422
    resp = await client.post("/v1/query", json={"question": "ok", "top_k": 21})
    assert resp.status_code == 422
...
async def test_stats_after_ingest(ingested_client):
    resp = await ingested_client.get("/v1/stats")
    assert resp.status_code == 200
    body = resp.json()
...
async def test_query_on_empty_index(client):
    resp = await client.post("/v1/query", json={"question": "anything at all?"})
    assert resp.status_code == 200
    body = resp.json()
...
async def test_reingest_replaces_document_chunks(ingested_client):
    ...
    stats = (await ingested_client.get("/v1/stats")).json()
    assert stats["documents"] == 3
    assert stats["chunks"] == 3

    # the chunk id is reused but must now serve the new content
    resp = await ingested_client.post(
        "/v1/query", json={"question": "How much water does a lungo use?", "top_k": 1}
    )
...
async def test_ingest_oversized_inputs_rejected_422(client):
    ...
    resp = await client.post(
        "/v1/ingest", json={"documents": [{"title": "big", "text": "x" * (MAX_TEXT_CHARS + 1)}]}
    )
    assert resp.status_code == 422
    # title over cap
    resp = await client.post(
...
async def test_ingest_blank_text_or_title_rejected_422(client):
    ...
    assert resp.status_code == 422
    resp = await client.post(
        "/v1/ingest", json={"documents": [{"title": "   ", "text": "real text"}]}
    )
    assert resp.status_code == 422
...
async def test_ingest_separator_only_text_accepted(client):
    ...
    resp = await client.post(
        "/v1/ingest", json={"documents": [{"title": "seps", "text": "\n\n. \n\n"}]}
    )
    assert resp.status_code == 200
...
async def test_query_oversized_question_rejected_422(client):
    resp = await client.post("/v1/query", json={"question": "q" * (MAX_QUESTION_CHARS + 1)})
    assert resp.status_code == 422
    # blank question also rejected
    resp = await client.post("/v1/query", json={"question": "   "})
    assert resp.status_code == 422
...
async def test_query_unicode_and_injection_are_inert(ingested_client):
    ...
    resp = await ingested_client.post(
        "/v1/query", json={"question": "café ☕ مرحبا שלום 你好 🚀 pgvector"}
    )
    assert resp.status_code == 200
    # SQL-injection-looking text is just tokens to the memory store: inert
    before = (await ingested_client.get("/v1/stats")).json()
    resp = await ingested_client.post(
        "/v1/query", json={"question": "'; DROP TABLE chunks; -- SELECT * FROM x"}
    )
    assert resp.status_code == 200
    after = (await ingested_client.get("/v1/stats")).json()
...
async def test_concurrent_ingest_and_query_keep_store_coherent(client):
    ...
    async def query(i: int):
        return await client.post("/v1/query", json={"question": f"alpha{i} beta{i}", "top_k": 5})
...
async def test_llm_failure_maps_to_502(store, embedder):
    ...
    async with httpx.AsyncClient(
        ...
        resp = await client.post("/v1/query", json={"question": "boom"})
